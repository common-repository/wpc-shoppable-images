...
class WPCleverWpcsi_Backend {
	...
	public function custom_column_value( $column, $post_id ) {
		...
		if ( $column == 'shortcode' ) {
            ...
            <div class="wpcsi-shortcode-col-wrap">
                <label>
                    <input type="text" onfocus="this.select();" readonly="readonly" value="[wpcsi id='<?php echo esc_attr( $post_id ); ?>']" class="code"/>
                </label>
            </div>
			<?php
		}
	...
	public function configuration_shortcode() {
        ...
        <div class="wpcsi-shortcode-col-wrap">
            <label>
                <input type="text" onfocus="this.select();" readonly="readonly" value="[wpcsi id='<?php echo esc_attr( get_the_ID() ); ?>']" class="code"/>
            </label>
        </div>
		<?php
	}
	...
	public function meta_item( $key = null, $tag = null ) {
		$label    = $tag['settings']['label'] ?? '';
		$lpos     = $tag['settings']['lpos'] ?? 'top-center';
		$content  = $tag['settings']['content'] ?? 'products';
		$position = $tag['settings']['position'] ?? 'top-center';
		$cart     = $tag['settings']['cart'] ?? 'no';
		$price    = $tag['settings']['price'] ?? 'yes';
		$carousel = $tag['settings']['carousel'] ?? 'no';
		$image    = $tag['settings']['image'] ?? 'left';
		$link     = $tag['settings']['link'] ?? 'same';
		$trigger  = $tag['settings']['trigger'] ?? 'click';
		$products = $tag['settings']['products'] ?? [];
		$text     = $tag['settings']['text'] ?? '';
		?>
        <div class="item-meta <?php echo esc_attr( isset( $key ) ? '' : 'active' ); ?>" id="wpcsi-meta-item-key-<?php echo esc_attr( $key ?? '{{(data.key - 1)}}' ); ?>" data-key="<?php echo esc_attr( $key ?? '{{(data.key - 1)}}' ); ?>">
            <form>
                <div class="header">
                    ...
                    <table class="wpcsi_configuration_table">
                        <tr class="wpcsi_configuration_tr">
                            ...
                            <td class="wpcsi_configuration_td">
                                <label> <input type="text" name="label" value="<?php echo esc_attr( $label ); ?>"/>
                                </label> <label> <select name="lpos">
